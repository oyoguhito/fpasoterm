...
Sync folder diagnostics are separate from terminal output logging. The titlebar `Log (^L)` button, or `Ctrl+Shift+L`, opens the log menu. `Start (^S)` / `Stop (^S)`, or `Ctrl+Shift+S`, records terminal output to a local log file or closes that file. `Show (^P)`, or `Ctrl+Shift+P`, opens a selector so you can choose which captured `terminal-*.log` to view. The log panel can delete the selected stopped log, or `Delete All` can empty the active log and delete all stopped `terminal-*.log` files after an in-panel confirmation. Saved log files are normalized for readability by removing common ANSI/control sequences, OSC sequences, and CR/control characters or replacing them with line breaks. By default logs are written under `~/.config/fpasoterm/User/logs`.
