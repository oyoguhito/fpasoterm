...
`lineHeight` defaults to `1` on macOS and `1.08` on Linux and Windows. The
extra lower margin on Linux and Windows keeps descenders such as `g`, `q`, and
`y`, plus `_`, visible. fpasoterm's bundled xterm.js accepts values down to
`0.5`. Existing shipped default values are migrated at runtime; explicit custom
values are retained.
