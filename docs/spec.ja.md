...
永続的な共有ストレージへ残さずにリモート側の出力を取得したい場合は、temporary web console を優先する方針です。これは sync folder とは別の、ユーザーが明示的に開始・停止する read-only の一時取得口です。

初期方針は `127.0.0.1` bind、random token、短い TTL、browser からの shell input 無効です。別端末から見る場合は SSH port forwarding、Tailscale、WireGuard、VPN などユーザーが管理する private path を使います。詳細は `docs/temporary-web-console.ja.md` に記録します。
...
- fpasoterm は一時的な web console を public network へ自動公開しません。
