...
`fpasoterm --config <path>` で一度だけ別の TOML file を使えます。`--width`、`--height`、`--size` は一度だけ window size を上書きします。
...
`config.toml` が存在しない場合、fpasoterm は既定値を `config.toml.example` として書き出します。既存のユーザー設定は上書きしません。

対応する設定 section:

- `window`: 初期ウィンドウサイズ、最小ウィンドウサイズ、背景色、theme source。
