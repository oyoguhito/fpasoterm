...
assert.match(rustMain, /strip_terminal_escape_sequences/);
assert.match(rustMain, /normalize_terminal_preview_lines/);
...
assert.match(indexHtml, />Log</);
assert.match(indexHtml, />Start</);
assert.match(indexHtml, />Show</);
...
assert.match(readme, /Linux builds enable GTK application-id matching/);
...
assert.match(syncDocsEn, /Log Show/);
...
assert.match(syncDocsJa, /Log Show/);
...
assert.match(renderer, /selectedDiagnosticsText/);
...
assert.match(renderer, /diagnostics copy failed/);
...
assert.match(renderer, /press Log Stop when finished/);
...
assert.doesNotMatch(renderer, /getSelection/);
...
assert.match(read('README.md'), /StartupWMClass=io\.github\.oyoguhito\.fpasoterm/);
assert.match(read('INSTALL.md'), /StartupWMClass=io\.github\.oyoguhito\.fpasoterm/);
