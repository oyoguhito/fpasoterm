...
assert.match(bin, /function pluginInstallPortIds\(value\)/);
assert.match(bin, /--plugin-install cannot contain duplicate ports/);
assert.match(bin, /checkout or its ports\/ directory/);
...
assert.match(bin, /launcher cargo build=incremental/);
assert.doesNotMatch(bin, /cargo-clean-start/);
assert.match(bin, /function bundledRustLldDirectory/);
assert.match(bin, /function localCargoBuildEnvironment/);
...
assert.doesNotMatch(installDesktop, /cargo clean/);
assert.match(installDesktop, /localCargoBuildEnvironment/);
assert.match(installDesktop, /link-arg=-fuse-ld=lld/);
...
assert.match(cargoToml, /\[profile\.dev\]/);
assert.match(cargoToml, /debug = 1/);
assert.match(cargoToml, /\[profile\.dev\.package\."\*"\]/);
...
assert.match(rustMain, /clipboard_write_osc52/);
...
assert.match(renderer, /writeOsc52Clipboard: \(text\) => window\.fpasoterm\.writeOsc52Clipboard\(text\)/);
...
assert.match(renderer, /writeClipboardText\(text\)\.then/);
assert.match(renderer, /terminalKeyboardCopyMark/);
assert.match(renderer, /setTerminalKeyboardCopyMark/);
assert.match(renderer, /selectTerminalKeyboardCopyRange/);
...
assert.match(renderer, /let terminalMirrorDismissed = false/);
assert.match(renderer, /closeTerminalMirrorButton\?\.addEventListener\('click'/);
assert.match(renderer, /enableFloatingPanelDrag\(\s*terminalMirrorElement/);
assert.match(renderer, /let diagnosticsPanelDismissed = false/);
assert.match(renderer, /function showDiagnosticsPanel\(\)/);
assert.match(renderer, /function closeDiagnosticsPanel\(\)/);
assert.match(renderer, /if \(!diagnosticsPanelDismissed\)/);
assert.match(renderer, /closeDiagnosticsButton\.addEventListener\('click', \(\) => \{\s*closeDiagnosticsPanel\(\);/);
...
assert.match(renderer, /function writeOsc52ClipboardText/);
assert.match(renderer, /navigator\.clipboard\.write\(\) pending/);
assert.match(renderer, /let lastOsc52ClipboardText = ''/);
assert.match(renderer, /function copyLastOsc52ToHostClipboard/);
assert.match(renderer, /terminalCopyLastOsc52Button\?\.addEventListener\('click'/);
assert.match(renderer, /OSC 52 clipboard received/);
...
assert.match(renderer, /Half-width kana: ｱｲｳｴｵ/);
