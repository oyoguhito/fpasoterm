...
assert.match(indexHtml, />Copy Ctrl\+Shift\+C</);
assert.match(indexHtml, />Paste Ctrl\+Shift\+V</);
...
assert.match(specEn, /Log menu also exposes `Copy` and `Paste` buttons/);
...
assert.match(specJa, /Log menu には keyboard 操作用の `Copy` \/ `Paste` button/);
