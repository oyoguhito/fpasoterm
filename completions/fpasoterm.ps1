...
$fpasotermCompletion = {
  ...
  $options = @(
    ...
    @('--plugin-disable', 'Alias for disable-plugin'), @('--plugin-run', 'Invoke a registered plugin command after launch'), @('--plugin-args', 'JSON value passed to plugin-run'), @('--plugin-enable-all', 'Enable all discovered plugins'), @('--plugin-disable-all', 'Disable all plugins'),
